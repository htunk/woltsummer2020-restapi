...
case class Location(latitude: Double, longitude: Double) {
  ...
  private def distanceInKm(loc1: Location, loc2: Location): Double = {
    ...
    val a = sin(diffLat / 2) * sin(diffLat / 2) + sin(diffLon / 2) * sin(diffLon / 2) * cos(radLat1) * cos(radLat2)
